...
public class ZombieController : MonoBehaviour
{
    ...
    public void takeDamage(int damage, float knockBack, Transform source)
    {
        ...
        Instantiate(damageNumber,transform.position,Quaternion.identity).GetComponent<dmgPopUp>().setup(damage, source.position); 
    ...
    void dealDamage()
    {
        target.GetComponent<TopDownHealthControll>().takeDamage(damage, transform, knockBack);
    }
    void dmgRange()
    {
        distance = Vector3.Distance(target.position, transform.position);
